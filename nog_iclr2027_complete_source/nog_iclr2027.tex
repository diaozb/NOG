...
\begin{document}
	...
	\begin{abstract}
		Recent progress in nonsmooth nonconvex optimization established non-asymptotic convergence to $(\delta,\epsilon)$-Goldstein stationary points with zeroth- or first-order oracles. In this paper, we study the parallel setting and propose the Nonconvex Optimistic Gradient (NOG) method. NOG finds a $(\delta,\epsilon)$-Goldstein stationary point in $\gO(d^{1/3}\delta^{-1}\epsilon^{-5/3})$ adaptive oracle rounds, improving the $\gO(\delta^{-1}\epsilon^{-3})$ depth of prior methods when $d=\gO(\epsilon^{-4})$, while retaining the state-of-the-art total oracle work. We instantiate NOG as communication-efficient distributed first- and zeroth-order algorithms and give quantum query implementations in the appendix. Controlled experiments on a nonsmooth nonconvex synthetic objective show that NOG's relative communication depth improves as the target accuracy becomes more stringent, consistent with the predicted separation in the $\epsilon$ dependence.
	\end{abstract}
	...
	consider the parallel setting, where the algorithm can simultaneously submit multiple queries $\{(\vx_1,\xi_1),\cdots, (\vx_Q,\xi_Q) \}$ at once, and then the zeroth- or first-order oracles return the function values or gradients at these points to the algorithm. Following the textbook \citep{nemirovski1994parallel}, the complexity of a parallel algorithm is measured by both \textit{depth} (the number of rounds the algorithm interacts with the oracles) and \textit{work} (the total number of queries, \textit{i.e.}, $Q \times$ depth). We ask a fundamental question:
	\begin{center}
	...
	Our numerical study complements these worst-case guarantees: on a controlled nonsmooth nonconvex objective, the communication-depth gap between NOG and the baselines improves as $\epsilon$ decreases, while the total training work remains of comparable finite-instance scale. Appendix~\ref{app:quantum} develops the quantum first- and zeroth-order implementations and their query complexities.
	...
	\begin{restatable}[Zeroth-order estimator] {prop}{propzoest}\label{prop:ZO-estimator}
		...
		\begin{align*}
			\mO_j(\vx) = 
			\frac{d}{2 \delta}\bigl(F(\vx + \delta \vv;\xi ) - F(\vx - \delta \vv;\xi)\bigr)\vv,
			\quad \vv \sim {\rm Unif}(\sS^{d-1}), \quad \xi \sim \Xi.
		\end{align*}
	\end{restatable}
	...
	Theorems \ref{thm:dist-ZO} and \ref{thm:dist-FO} show that our proposed method has lower depth than existing ones for nonsmooth nonconvex optimization. In the next section, we show that this also gives more efficient algorithms in a setting that favors parallel computing.
...
We test whether the communication-depth separation predicted by
Theorems~\ref{thm:dist-FO} and~\ref{thm:dist-ZO} is visible on a controlled
nonsmooth nonconvex problem.  For $\xi\in\{1,\ldots,n\}$, let
\begin{equation}\label{eq:synthetic-objective}
	F(\vx;\xi)=\max_{1\leq r\leq R}
	\sin(\mathbf{a}_{\xi,r}^{\top}\vx+b_{\xi,r})
	+\lambda\lVert\vx\rVert_1 .
\end{equation}
We use $d=100$, $n=4096$, $R=1$, $\lambda=10^{-3}$, and eight workers.
The sinusoidal term is nonconvex and the $\ell_1$ term is nonsmooth.  Depth is
the number of sequential communication/oracle layers; work is the total number
of training oracle calls across workers.  At common checkpoints we estimate a
smoothed-gradient norm using a method-independent high-precision sample bank.
A target is reached only after two consecutive evaluations are at most
$\epsilon$.  Parameters are selected on pilot seeds $100$--$104$, frozen, and
then evaluated on formal seeds $0$--$19$.  Evaluation calls are excluded from
training work.  Appendix~\ref{app:numerical-experiments} gives the estimators,
parameters, complete protocol, and audit information.

\subsection{First-order oracles}\label{sec:experiments-fo}
We compare NOG-FO with ME-DOL-FO \citep{sahinoglu2024online}.
Figure~\ref{fig:fo-first-hit} uses the homogeneous batch-8 regime
$\epsilon\in[0.0105,0.2]$; all method--threshold pairs have 20/20 confirmed
hits.  As $\epsilon$ decreases, the paired depth ratio
$D_{\mathrm{ME\text{-}DOL}}/D_{\mathrm{NOG}}$ grows monotonically from $0.488$
to $1.102$, crossing one at the most stringent targets.  At $\epsilon=0.0105$,
NOG requires mean depth $587.2$ versus $642.9$ for ME-DOL, with mean work
$4697.6$ versus $5143.2$.  Across the displayed range, the paired NOG/ME-DOL
work ratio stays between $0.980$ and $2.049$.  Thus the finite experiment
exhibits the predicted improvement in \emph{relative} depth as accuracy
increases, without an order-of-magnitude work separation.  It does not show
that NOG is uniformly better at coarse accuracy.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/fo_first_hit_depth_work.pdf}
\caption{First-order first-hit complexity on
Eq.~(\ref{eq:synthetic-objective}).  Lines are means over 20 frozen formal
seeds and bands are 95\% $t$ confidence intervals.  Left: communication depth.
Right: total training SFO work.  The displayed interval keeps the per-layer
batch fixed for both methods; all points have 20/20 confirmed hits.}
\label{fig:fo-first-hit}
\end{figure}

\subsection{Zeroth-order oracles}\label{sec:experiments-zo}
We compare NOG-ZO with ME-DOL-ZO \citep{sahinoglu2024online}, DGFM, and DGFM+
\citep{lin2024decentralized}.  All methods use two-point estimators and the
same cap of $983{,}040$ training function evaluations per seed.
Figure~\ref{fig:zo-first-hit} restricts attention to the common
$\epsilon\in[0.03,0.2]$ interval where all four methods have 20/20 hits.  NOG
has the smallest mean depth at every displayed target.  Moreover, as
$\epsilon$ decreases from $0.2$ to $0.03$, the paired depth ratios
ME-DOL-ZO/NOG-ZO, DGFM/NOG-ZO, and DGFM+/NOG-ZO grow from
$1.596$, $3.556$, and $3.520$ to $2.717$, $7.210$, and $11.387$,
respectively.  At $\epsilon=0.03$, NOG's mean depth is $229.2$, compared with
$622.8$, $1652.4$, and $2609.6$ for the three baselines.  The corresponding
works are $234{,}701$, $159{,}437$, $211{,}507$, and $355{,}302$: NOG is
communication-efficient but not uniformly work-minimal.  This is qualitative
evidence for the improved $\epsilon$ dependence of NOG's depth, rather than
an empirical identification of the worst-case exponent.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{figures/zo_first_hit_depth_work.pdf}
\caption{Zeroth-order first-hit complexity on
Eq.~(\ref{eq:synthetic-objective}).  Lines are means over 20 frozen formal
seeds and bands are 95\% $t$ confidence intervals.  Left: communication depth.
Right: total training SZO work.  The plot shows the common interval on which
every method--threshold pair has 20/20 confirmed hits.}
\label{fig:zo-first-hit}
\end{figure}

\paragraph{Scope.}
The experiments isolate the depth--work tradeoff on a problem satisfying the
paper's nonsmooth nonconvex setting.  Worst-case upper bounds need not be tight
on one finite instance, so we claim agreement only in the direction of the
relative depth trend.  We neither fit the theoretical exponents as empirical
equalities nor claim universal dominance across objectives.

\paragraph{Quantum extension.}
The same NOG framework also admits quantum implementations based on quantum
mean estimation.  Their oracle models, algorithms, query complexities, and
proofs are deferred to Appendix~\ref{app:quantum}.
...
\section{Conclusions and Future Works}
	...
	We introduced NOG, a meta-algorithm for highly parallel nonsmooth nonconvex
	optimization.  Its first- and zeroth-order instantiations improve the
	communication-depth dependence on the target stationarity while retaining
	state-of-the-art total oracle work.  On a controlled synthetic objective,
	the relative depth of NOG improves as $\epsilon$ decreases in both oracle
	models, while the work comparisons remain instance- and method-dependent.
	These experiments support the predicted communication trend but do not
	establish exact worst-case exponents.

	Important open questions include matching lower bounds for parallel
	nonsmooth nonconvex optimization and decentralized implementations that do
	not require a centralized server
	\citep{lin2024decentralized,sahinoglu2024online,chen2026decentralized}.
	For the quantum extensions in Appendix~\ref{app:quantum}, the common
	$\delta^{-1}\epsilon^{-7/3}$ dependence also leaves open whether faster
	query algorithms are possible.
